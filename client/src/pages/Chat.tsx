      ...
      <div className={`max-w-[80%] flex flex-col gap-2 ${isUser ? 'items-end' : 'items-start'}`}>
        <div className={`rounded-2xl px-4 py-3 font-body text-sm leading-relaxed
          ${isUser
            ...
            : 'bg-card border border-border-dim text-text-muted rounded-tl-sm'
          }`}>
...
export default function Chat() {
  ...
  // Pre-populate with mock chat history
  useEffect(() => {
    ...
    <div className="flex h-[calc(100vh-8rem)] gap-4">
        ...
        <div className="px-5 pb-5 border-t border-border-dim pt-3">
          <div className="flex gap-3">
            ...
            <button
              ...
              className="btn-primary self-end px-4 disabled:opacity-40 disabled:cursor-not-allowed"
            >
              {isStreaming ? '⏳' : '→'}
            </button>
          </div>
          <p className="font-mono text-xs text-text-faint mt-2">
            Enter to send · Shift+Enter for new line · I only answer from your records
          </p>
        </div>
      </div>
      ...
      <div className="w-56 flex flex-col gap-4 hidden xl:flex">
        <div className="mv-card flex flex-col gap-3">
          ...
          {messages.filter((m) => m.role === 'assistant' && m.sourceDocIds.length > 0).slice(-1).map((m, i) => (
